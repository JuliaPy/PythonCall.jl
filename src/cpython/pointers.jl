const CAPI_FUNC_SIGS = Dict{Symbol, Pair{Tuple, Type}}(
    ...
    # CAPSULE
    :PyCapsule_New => (Ptr{Cvoid}, Ptr{Cuchar}, Ptr{Cvoid}) => PyPtr,
    :PyCapsule_GetName => (PyPtr,) => Ptr{Cuchar},
    :PyCapsule_SetName => (PyPtr, Ptr{Cuchar}) => Cint,
    :PyCapsule_GetPointer => (PyPtr, Ptr{Cuchar}) => Ptr{Cvoid},
