...
function PyArray{T,N,R,M,L}(o::PyRef, info) where {T,N,R,M,L}
    ...
    !L ||
        N ≤ 1 ||
        size_to_fstrides(bytestrides[1], size...) == bytestrides ||
        error("not linearly indexable, got L=$L")
...
PyArray{T,N,R,M}(o::PyRef, info) where {T,N,R,M} = PyArray{
    T,
    N,
    R,
    M,
    N ≤ 1 || size_to_fstrides(info.bytestrides[1], info.size...) == info.bytestrides,
}(
    o,
    info,
)
...
PyArray{<:Any,N}(o::PyRef, info) where {N} =
    PyArray{pyarray_default_T(info.eltype),N}(o, info)
...
function pyarray_info(ref; buffer = true, array = true, copy = true)
    ...
    if array && copy && pyhasattr(ref, "__array__")
        try
            return pyarray_info(
                pycall(PyRef, pygetattr(PyRef, ref, "__array__"));
                buffer = buffer,
                array = array,
                copy = false,
            )
...
pyarray_offset(x::PyArray{T,1,R,M,true}, i::Int) where {T,R,M} = (i - 1) .* x.bytestrides[1]
