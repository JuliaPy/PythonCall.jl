...
pycomplex(x) = ispy(x) ? pybuiltins.complex(x) : pycomplex(convert(Complex{Cdouble}, x))
pycomplex(x, y) = (ispy(x) || ispy(y)) ? pybuiltins.complext(x, y) : pycomplex(convert(Cdouble, x), convert(Cdouble, y))
export pycomplex
