...
# needed for static compilation of Python.

# As a result, if you switch to a different version or path of Python, you
# will probably need to re-run Pkg.build("Python").
...
const prefsfile = joinpath(first(DEPOT_PATH), "prefs", "Python")
...
try # make sure deps.jl file is removed on error
        ...
        if Sys.ARCH in (:i686, :x86_64)
            if isa(e1, UseCondaPython)
                @info string("Using the Python distribution in the Conda package by default.\n",
                     "To use a different Python version, set ENV[\"PYTHON\"]=\"pythoncommand\" and re-run Pkg.build(\"Python\").")
            else
    ...
    @info "Python is using $python (Python $pyversion) at $programname, libpython = $libpy_name"

    if pyversion < v"2.7"
        error("Python 2.7 or later is required for Python")
    end

    writeifchanged("deps.jl", """
    const PYEXE = "$(escape_string(python))"
    const PYLIB = "$(escape_string(libpy_name))"
    const PYPROGNAME = "$(escape_string(programname))"
    const PYVERSION = $(repr(pyversion))
    const PYHOME = "$(escape_string(PYTHONHOME))"

    "True if we are using the Python distribution in the Conda package."
    const USING_CONDA = $use_conda
    """)
    ...
    # remove deps.jl (if it exists) on an error, so that Python will
